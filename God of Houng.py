...
joke_init_img =  pygame.image.load(os.path.join("nn's","init.png")).convert()
player_img = pygame.image.load(os.path.join("nn's","n1.png")).convert()
player_mini_img = pygame.transform.scale(player_img,(25,19))
player_mini_img.set_colorkey(BLACK)
#小圖示
# pygame.display.set_icon(player_mini_img)
nn_icon = pygame.image.load(os.path.join("nn's","nnicon2.png")).convert()
pygame.display.set_icon(nn_icon)
...
for i in range(2):
    expl_img =  pygame.image.load(os.path.join("img",f"joke_expl{i}.png")).convert()
    expl_img.set_colorkey(BLACK)
    expl_anim["joke"].append(pygame.transform.scale(expl_img,(50,50)))
...
def draw_text(surf,text,size,x,y):
    ...
    text_surface = font.render(text,True,GREEN) #render渲染, True:使用反鋸齒
...
def draw_init():
    screen.blit(joke_init_img,(0,0)) 
    draw_text(screen,Game_name,64,WIDTH/2,HEIGHT*0.10)
    draw_text(screen,"← → : 恩皇左欺敵、右欺敵",22,WIDTH/2,HEIGHT*0.7)
    draw_text(screen,"Space : 恩皇請你吃慶記",22,WIDTH/2,HEIGHT*0.8)
    draw_text(screen,"隨便按就開始",18,WIDTH/2,HEIGHT*0.9)
...
class Joke_Expl(pygame.sprite.Sprite): 
    def __init__(self, center, size): 
        pygame.sprite.Sprite.__init__(self)
        self.size = size
...
while running:
    ...
    for hit in hits : #撞到飛船的所有石頭
        ...
        if player.health <= 0:
            ...
            joke_expl = Joke_Expl(player.rect.center,"joke")
            all_sprites.add(death_expl)
            all_sprites.add(joke_expl)
